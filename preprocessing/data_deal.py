...
from preprocessing.data_compress import compress
...
def train_val_split(X, y, valid_size=0.1, random_state=2019, shuffle=True):
    ...
    train, valid = [], []
    bucket = [[] for _ in args.answer_type]
            ...
            for qa in qas:
                ...
                if is_impossible == "true" or len(answers) == 0:
                    ...
                    for answer in answers:
                        ...
                        if short_context == sample["context"]:
                            compress_num += 1
                        samples.append(sample)

    print("压缩了 {} 样本".format(compress_num))
    if is_training:
...
def convert_examples_to_features(examples,
    ...
    for example_index, example in enumerate(examples):
        ...
        orig_to_tok_index = []
        all_doc_tokens = []
        for (i, token) in enumerate(example.doc_tokens):  #doc_token: context
        ...
        #print(doc_spans, len(all_doc_tokens), len(query_tokens))
#[DocSpan(start=0, length=484), DocSpan(start=484, length=484), DocSpan(start=968, length=23)] 991 25
